...
function buildNaicsSearchPaths({ samAssignedNaics, derivedPrimaryNaics, derivedRelatedNaics }) {
  ...
  let relatedPriority = paths.length + 1;
  related.forEach(code => {
    if (add(code, 'derived_related', relatedPriority)) relatedPriority += 1;
  });
  return paths;
}
...
function buildContractDna({ opportunity = {}, derivation = {}, requirements = {} } = {}) {
  ...
  return {
    ...
    competition: {
      ...
      eligible_business_classification: clean(first(requirements.eligible_business_classification, opportunity.eligible_business_classification)) || null,
    },
    requirement: {
...
function buildBusinessSearchDna(contractDna = {}) {
  ...
  const naicsPaths = Array.isArray(classification.naics_search_paths) ? classification.naics_search_paths : buildNaicsSearchPaths({
  ...
  return {
    schema_version: 'ngcc-business-search-dna-v1', contract_notice_id: contractDna.notice_id || null,
    contract_title: contractDna.title || null, source: 'SAM.gov Entity Management',
    hard_requirements: {
      ...
      no_known_hard_disqualifier: true, registration_valid_for_pursuit: true,
    },
    retrieval: {
      naics_search_paths: naicsPaths, search_naics: naicsPaths.map(path => path.naics_code),
      capability_terms: cleanArray(requirement.required_capabilities), products_services: cleanArray(requirement.products_services),
      procurement_keywords: cleanArray(classification.procurement_keywords), procurement_language: cleanArray(classification.procurement_language),
      psc: classification.psc || null, supplier_role: requirement.supplier_role || null,
      place_of_performance: requirement.place_of_performance || null,
    },
    qualification: {
      compare_exact_naics: true, compare_related_naics: true, compare_capability_alignment: true,
      compare_supplier_role: true, compare_set_aside_eligibility: true, compare_psc_alignment: Boolean(classification.psc),
      compare_semantic_evidence: true, compare_geography_capacity: Boolean(requirement.place_of_performance),
    ...
    output_contract: { ranked_candidates: true, explainable_match_evidence: true, search_path_attribution: true, operator_review_required: true },
