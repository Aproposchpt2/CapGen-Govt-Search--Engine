...
function strongestPath(candidate = {}) {
  ...
  return paths.slice().sort((a, b) => pathScore(b) - pathScore(a) || Number(a.priority || 99) - Number(b.priority || 99))[0] || null;
}
...
function verificationItems(contractDna = {}, searchDna = {}, candidate = {}) {
  ...
  const qualification = searchDna.qualification || {};
  const items = [];
  ...
  if (geographicRestrictions.length) {
    const state = candidate.state || '';
    const matched = containsStateRestriction(geographicRestrictions, state);
    items.push({
      code: 'GEOGRAPHIC_RESTRICTION',
      requirement: geographicRestrictions.join('; '),
      status: matched ? 'SUPPORTED' : 'UNVERIFIED',
      reason: matched ? `Candidate state ${state} appears in the stated geographic restriction evidence.` : 'Candidate business location does not by itself prove compliance with the stated place-of-performance or geographic restriction.',
    });
  }
  return items;
}
...
function qualifyCandidate({ candidate = {}, contractDna = {}, businessSearchDna = {} } = {}) {
  ...
  if (strongest) {
    const label = strongest.source === 'sam_assigned' ? 'SAM-assigned NAICS' : strongest.source === 'derived_primary' ? 'requirements-derived primary NAICS' : 'requirements-derived related NAICS';
    reasons.push(`Candidate was discovered through the ${label} search path (${strongest.naics_code}).`);
    evidence.push({ type: 'NAICS_SEARCH_PATH', source: strongest.source, value: strongest.naics_code, weight_awarded: signals.naics_path });
  }

  signals.multiple_search_paths = paths.length > 1 ? WEIGHTS.multipleSearchPaths : 0;
  if (signals.multiple_search_paths) {
    reasons.push(`Candidate appeared through ${paths.length} independent approved NAICS search paths.`);
    evidence.push({ type: 'MULTI_PATH_CORROBORATION', value: paths.map(p => p.naics_code), weight_awarded: signals.multiple_search_paths });
  }

  // Stage 04 searches only active SAM registrations. Preserve that as explicit evidence.
  signals.active_sam_registration = WEIGHTS.activeSamRegistration;
  reasons.push('Candidate was returned by the active-registration SAM.gov Entity Management search.');
  evidence.push({ type: 'ACTIVE_SAM_REGISTRATION', source: 'SAM.gov Entity Management', value: true, weight_awarded: signals.active_sam_registration });
  ...
  if (signals.identity) {
    reasons.push('Candidate has both UEI and CAGE identifiers in the SAM entity response.');
    evidence.push({ type: 'FEDERAL_IDENTITY', value: { uei: candidate.ueiSAM || candidate.uei, cage: candidate.cageCode || candidate.cage_code }, weight_awarded: signals.identity });
  }
  ...
  const verification = verificationItems(contractDna, businessSearchDna, candidate);
  const unresolved = verification.filter(item => item.status === 'UNVERIFIED');
  const explicitMismatch = verification.filter(item => item.status === 'MISMATCH');
  const score = explicitMismatch.length ? 0 : Math.min(100, Object.values(signals).reduce((sum, value) => sum + Number(value || 0), 0));

  let qualificationStatus;
  if (explicitMismatch.length) qualificationStatus = 'DISQUALIFIED';
  else if (unresolved.length) qualificationStatus = 'REVIEW_REQUIRED';
  else if (score >= 70) qualificationStatus = 'QUALIFIED';
  ...
  return {
    ...
    aoie_lineage: { engine_version: AOIE_ENGINE_VERSION, scoring_version: AOIE_SCORING_VERSION, pattern: 'weighted_signals+hard_gates+explainable_evidence' },
...
function rankCandidates({ candidates = [], contractDna = {}, businessSearchDna = {} } = {}) {
  return (Array.isArray(candidates) ? candidates : [])
    ...
    .sort((a, b) => b.qualification_score - a.qualification_score || a.business_name.localeCompare(b.business_name))
    .map((candidate, index) => ({ rank: index + 1, ...candidate }));
}

function qualificationSummary(ranked = []) {
  const counts = { QUALIFIED: 0, REVIEW_REQUIRED: 0, DISQUALIFIED: 0 };
  ...
  return {
    ...
    top_score: ranked[0]?.qualification_score ?? null,
  };
}

module.exports = { QUALIFICATION_VERSION, WEIGHTS, qualifyCandidate, rankCandidates, qualificationSummary };
