// RFCP — recipient-facing unsubscribe link (clicked from outside the site,
...
function page(title, body, tone) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${title} | NGCC</title></head><body style="margin:0;background:#0F2A6A;color:#fff;font-family:Arial,Helvetica,sans-serif"><main style="min-height:100vh;display:grid;place-items:center;padding:24px"><section style="width:min(560px,100%);background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.14);border-radius:14px;padding:38px 34px;border-top:3px solid ${accent}"><div style="font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:rgba(255,255,255,.4);margin-bottom:12px">Registered Federal Contractors Portal</div><h1 style="margin:0 0 14px;font-size:28px;font-weight:400">${title}</h1>${body}</section></main></body></html>`;
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('[rfcp-unsubscribe]', error.message);
