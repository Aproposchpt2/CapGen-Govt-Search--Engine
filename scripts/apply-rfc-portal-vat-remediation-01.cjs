...
// ---------------------------------------------------------------------------
// 1) Returning-member UX: paste is allowed; do not present a fake restriction.
// ---------------------------------------------------------------------------
let onboarding = read('onboarding.html');
onboarding = onboarding.replace(
  /<p style="font-size:\.72rem;color:rgba\(255,255,255,\.38\);margin:-8px 0 12px;line-height:1\.5">⌨ Please <strong style="color:rgba\(255,255,255,\.55\)">type<\/strong> your email address — do not paste\.<\/p>/,
  '<p style="font-size:.72rem;color:rgba(255,255,255,.38);margin:-8px 0 12px;line-height:1.5">Enter or paste your registered business email.</p>'
);
onboarding = onboarding.replace(
  /<p style="font-size:\.72rem;color:rgba\(255,255,255,\.38\);margin:-8px 0 12px;line-height:1\.5">⌨ Please <strong style="color:rgba\(255,255,255,\.55\)">type<\/strong> the 6-digit code — do not paste\.<\/p>/,
  '<p style="font-size:.72rem;color:rgba(255,255,255,.38);margin:-8px 0 12px;line-height:1.5">Enter or paste the 6-digit access code.</p>'
);
write('onboarding.html', onboarding);

// ---------------------------------------------------------------------------
// 2) Session bridge: profile-cookie session first; valid pipeline bearer token
//    is a secure returning-member fallback to the latest verified shared profile.
// ---------------------------------------------------------------------------
...
if (!profileSession.includes('RFC_PORTAL_PIPELINE_SESSION_BRIDGE_V1')) {
  ...
  // Returning-member OTP login uses client_sessions. Resolve that authenticated
  // server-side session to the latest verified shared Nat-Corp/NGCC profile.
  ...
  const clientRows = await db(
    'client_sessions',
    'GET',
    \`?session_token=eq.\${encodeURIComponent(bearer)}&select=email,expires_at&limit=1\`,
  );
...
// ---------------------------------------------------------------------------
// 3) State matching safety gate: only presentation-ready released inventory.
// ---------------------------------------------------------------------------
let stateMatches = read('netlify/functions/ngcc-state-matches.mjs');
stateMatches = stateMatches.replace(
...
stateMatches = stateMatches.replace(
  ...
  "const SELECT = 'select=id,title,description,agency:issuing_organization,solicitation_number:solicitation_number,state_code,jurisdiction_name,place_of_performance_county,naics_codes,procurement_type,response_deadline,posted_at,source_url,official_source_url,acquisition_method,package_document_count,match_readiness_status';"
);
stateMatches = stateMatches.replace(
  `      title: row.title,
      agency: row.agency,`,
...
// ---------------------------------------------------------------------------
// 4) Merged dashboard: consume the shared state matcher instead of legacy NGEM.
// ---------------------------------------------------------------------------
...
dashboard = dashboard.replace(
  '<div id="nv-service-panel" style="background:rgba(255,255,255,.04);',
  '<div id="nv-service-panel" style="display:none;background:rgba(255,255,255,.04);'
);

dashboard = dashboard.replace(
  /  <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;background:rgba\(255,255,255,\.03\);border:1px solid var\(--line\);border-radius:12px;padding:12px 16px;margin-bottom:18px">\s*<span style="font-size:\.82rem;color:var\(--muted\)">Want the full standalone Nevada experience — saved preferences, email alerts, and more\?<\/span>\s*<a id="nv-explore-link"[\s\S]*?<\/a>\s*<\/div>/,
...
const oldLoadNevada = `function loadNevada() {
  loadNvServicesFromStorage(); // restore saved service selections
  fetch('/.netlify/functions/ngem-pipeline')
    .then(function(r) { return r.json(); })
    .then(function(d) {
      nvAll = Array.isArray(d.bids) ? d.bids : (Array.isArray(d.results) ? d.results : []);
  ...
  fetch('/api/state-matches', { headers: { Authorization: 'Bearer ' + tok } })
    ...
    .then(function(result) {
      ...
      nvAll = (Array.isArray(d.results) ? d.results : []).map(function(row) {
        return {
          id: row.internal_id,
          bid_id: row.internal_id,
          title: row.title,
          description: row.description || '',
          agency: row.agency,
          solicitation_no: row.solicitation_number,
          type: row.procurement_type || 'Contract',
          close_date: row.response_deadline,
          issue_date: row.posted_at,
          url: row.source_url,
...
dashboard = replaceRequired(dashboard, oldLoadNevada, newLoadNevada, 'state pipeline loader');

dashboard = dashboard.replace(
  "var hay = (String(b.title||'') + ' ' + String(b.agency||'')).toLowerCase();",
  "var hay = (String(b.title||'') + ' ' + String(b.agency||'') + ' ' + String(b.state_code||'') + ' ' + String(b.jurisdiction_name||'') + ' ' + String(b.description||'')).toLowerCase();"
);
dashboard = dashboard.replace(
  "+ '<div class=\"nv-meta\"><span>' + esc(b.agency||'—') + '</span>'",
